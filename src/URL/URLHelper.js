...
export class URLHelper {
  ...
  static addQueryParam(param, value, location, history, shouldReplace=false) {
    const params = URLHelper.getSearchObject(location);
    params[param] = value;

    const func = shouldReplace ? history.replace : history.push;
  ...
  static removeQueryParam(param, location, history, shouldReplace=false) {
    const params = URLHelper.getSearchObject(location);
    delete params[param];
